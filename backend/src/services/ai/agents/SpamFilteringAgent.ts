...
export class SpamFilteringAgent extends AIAgentBase<ISpamFilteringResult> {
  ...
  /**
   * Extract GitHub repository URL from submission URL
  ...
  private extractGitHubUrl(submissionUrl: string): string | null {
    try {
      ...
      for (const domain of SPAM_PATTERNS.SUSPICIOUS_DOMAINS) {
        if (submissionUrl.includes(domain)) {
      ...
      // Handle URLs that might contain GitHub but are not repository URLs
      if (submissionUrl.includes('gist.github.com')) {
      ...
      try {
        ...
        // Try adding http:// if the URL doesn't have a protocol
        if (!submissionUrl.startsWith('http://') && !submissionUrl.startsWith('https://')) {
          try {
            url = new URL(`https://${submissionUrl}`);
          } catch (error) {
            return null;
          }
        } else {
      ...
      // Normalize the URL to the repository root
      const pathParts = url.pathname.split('/').filter(part => part.length > 0);
      ...
      // Handle GitHub URLs that point to specific files, branches, etc.
      // We always normalize to the repository root
